...
class Allowance{
	...
	public static function get($allowanceId, $field){  // getting row from allowance table by allowace id
		$query = Db::fetch("allowances", "$field", "a_id = ? ", $allowanceId, "", "", "" );
		$data = Db::num($query); 
		return $data[0]; 
	}

	public static function getid($aname, $field){  // getting row from allowance table by allowace name
		$query = Db::fetch("allowances", "$field", "allowance_name = ? ", $aname, "", "", "" );
		$data = Db::num($query); 
		return $data[0]; 
	}
	
	public static function delete($allowanceId){  //delete an allowance from table
