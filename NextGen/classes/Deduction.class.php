...
class Deduction{
	...
	public static function get($deductionId, $field){ // getting row from allowance table by deduction id
		$query = Db::fetch("deductions", "$field", "d_id = ? ", $deductionId, "", "", "" );
		$data = Db::num($query); 
		return $data[0]; 
	}

	public static function getid($dname, $field){ // getting row from allowance table by deduction name
		$query = Db::fetch("deductions", "$field", "deduction_name = ? ", $dname, "", "", "" );
		$data = Db::num($query); 
		return $data[0]; 
	}
	
	public static function delete($deductionId){ //delete an deduction from table
